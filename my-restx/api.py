...
# List headers that should never be handled by my-restx
...
class Api(object):
    ...
    def error_router(self, original_handler, e):
        """
        This function decides whether the error occurred in a my-restx
        endpoint or not. If it happened in a my-restx endpoint, our
        handler will be dispatched. If it happened in an unrelated view, the
        app's original error handler will be dispatched.
        In the event that the error occurred in a my-restx endpoint but
    ...
    def _deferred_blueprint_init(self, setup_state):
        ...
        if not setup_state.first_registration:
            raise ValueError("my-restx blueprints can only be registered once.")
    ...
    def unauthorized(self, response):
        ...
        if self.serve_challenge_on_401:
            realm = current_app.config.get("HTTP_BASIC_AUTH_REALM", "my-restx")
